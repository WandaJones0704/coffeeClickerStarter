...
let big_coffeeParent = document.getElementById("big_coffee");
...
big_coffeeParent.addEventListener("click", clickCoffee);
...
function clickCoffee(clickdata) {
  // We update the counter when clicked
...
function unlockProducers(producers, coffeeCount) {
  ...
  for (let i = 0; i < producers.length; i++) {
    ...
    if (coffeeCount >= producer.price / 2) {
      console.log("Unlocked this producer " + producer.id);
...
function getUnlockedProducers(data) {
  ...
  let unlocked_producers = producers.filter(function (producer) {
    return producer.unlocked;
  });
  return unlocked_producers;
...
function renderProducers(data) {
  console.log("called renderProducer");
  ...
  let unlocked_producers = getUnlockedProducers(data);
  for (let i = 0; i < unlocked_producers.length; i++) {
    unlocked_producer = unlocked_producers[i];

    buy_id = `buy_${unlocked_producer.id}`;

    console.log("buy_id " + buy_id);

    producer_buy_button = document.getElementById(buy_id);

    console.log("buy button object" + producer_buy_button);

    producer_buy_button.addEventListener("click", buyButtonClick);
  }
}
...
function buyButtonClick(event, data) {
  ...
  if (event.target.tagName === "BUTTON") {
    ...
    const result = attemptToBuyProducer(window.data, producerId);
    if (!result) {
      window.alert("☕️Keep clicking, not enough coffee☕️");
    } else {
